...
const LandingPage: React.FC<LandingPageProps> = ({ onStart }) => (
  <div className="min-h-screen bg-[#FDFDFF] overflow-x-hidden selection:bg-indigo-100 selection:text-indigo-900">
    <nav className="fixed top-0 inset-x-0 h-20 bg-white/70 backdrop-blur-xl border-b border-slate-200/50 z-[100] transition-all">
      <div className="max-w-7xl mx-auto px-6 h-full flex items-center justify-between">
        ...
        <div className="hidden md:flex gap-10 items-center">
          {["Product", "Intelligence", "Enterprise", "Careers"].map((item) => (
            <a
              key={item}
              href="#"
    ...
    <header className="relative pt-44 pb-32 px-6">
      ...
      <div className="max-w-7xl mx-auto text-center">
        <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-white border border-slate-200 rounded-full text-[10px] font-black tracking-widest text-slate-400 uppercase mb-8 shadow-sm">
          <Sparkles size={12} className="text-amber-500" /> Powered by Gemini
          3.0 Multimodal
        </div>
        <h1 className="text-6xl md:text-[100px] font-black text-slate-900 leading-[0.85] tracking-tight mb-8">
          RETHINK THE <br />
        ...
        <p className="text-xl md:text-2xl text-slate-500 max-w-2xl mx-auto mb-12 font-medium leading-relaxed">
          The high-performance matching engine that quantifies potential across
          5 dimensions of professional compatibility.
        </p>
        <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
          <button
            onClick={() => onStart("signup")}
            className="px-10 py-5 bg-indigo-600 text-white rounded-[24px] font-black text-lg shadow-2xl shadow-indigo-200 hover:bg-indigo-700 hover:-translate-y-1 transition-all flex items-center gap-3"
          >
            Get Started Now <ArrowRight size={20} />
          </button>
          <button className="px-10 py-5 bg-white border border-slate-200 text-slate-800 rounded-[24px] font-black text-lg hover:bg-slate-50 transition-all">
            Watch Engine Demo
          </button>
        </div>
      </div>
